...
%adapted by Daniel Robles to convert Malawi Muse files to github. April 2023. Edits made by Daniel Robles are signed as DR.    
...
addpath(fullfile(fileparts(which('convert_MUSE_to_BV.m')),'./Other Files')); %Add the path for supporting files %the 
...
%% Finding files
prefix = cell2mat(inputdlg('Enter the prefix of your participant filename','Peer to BV',1,{'Participant_'})); %Input participant file name %For malawi data, use 'eeg_' as the prefix - DR
...
for counter = 1:length(filenames) %Number of participants - if this is incorrect make sure the skipped files was correct (line 38 to 44)
    ...
    [EEG.data] = csvdata(:,2:6)'; %Create variable for EEG data %changed to 6 to include aux channel. DR. 
    ...
    [EEG.nbchan] = 5; %Number of electrodes %changed to 5 to include Pz - DR. 
